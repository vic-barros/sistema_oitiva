...
public class Servidor {
	...
	private String montarJsonOitiva(Oitiva o, int indice) {
		return "{" + "\"indice\":" + indice + "," + "\"idOitiva\":" + o.getIdOitiva() + "," + "\"nomeDepoente\":\""
				+ o.getPessoaIntimada().getNome() + "\"," + "\"cpfDepoente\":\""
				+ (o.getPessoaIntimada().getCpf() != null ? o.getPessoaIntimada().getCpf() : "") + "\","
				+ "\"tipoPessoa\":\"" + o.getPessoaIntimada().getTipoDePessoa() + "\"," + "\"numOcorrencia\":"
				+ o.getProcedimento().getNumeroOcorrencia() + "," + "\"anoOcorrencia\":"
				+ o.getProcedimento().getAnoOcorrencia() + "," + "\"crime\":\"" + o.getProcedimento().getCrime() + "\","
				+ "\"dia\":" + o.getDataHora().getDayOfMonth() + "," + "\"mes\":" + o.getDataHora().getMonthValue()
				+ "," + "\"ano\":" + o.getDataHora().getYear() + "," + "\"hora\":" + o.getDataHora().getHour() + ","
				+ "\"minuto\":" + o.getDataHora().getMinute() + "," + "\"funcionarioResponsavel\":\""
				+ o.getFuncionarioResponsavel().getNome() + "\"," + "\"status\":\"" + o.getStatus() + "\","
	...
	private void handleOitivas(HttpExchange ex) throws IOException {
		...
		if (ex.getRequestMethod().equalsIgnoreCase("GET")) {
			...
			try {
				...
				Depoente depoente = new Depoente(nomeDepoente, cpfDepoente.isBlank() ? null : cpfDepoente, tipo);
	...
	private void handleAdmin(HttpExchange ex) throws IOException {
		...
		// Extrai qual ação admin quer fazer pela URL
		// ex: /admin?acao=pendentes, /admin?acao=aprovar, /admin?acao=resetarSenha
		String query = ex.getRequestURI().getQuery();
		String acao = extrairParametroUrl(query, "acao");
		...
		try {
			...
			if (admin == null || !admin.isAdmin()) {
				enviarResposta(ex, 403, "{\"sucesso\":false,\"erro\":\"Acesso negado\"}");
				return;
			}

			switch (acao) {
			case "pendentes" -> {
				ArrayList<FuncionarioDelegacia> pendentes = funcionarioDAO.listarPendentes();
				StringBuilder sb = new StringBuilder("[");
				for (int i = 0; i < pendentes.size(); i++) {
					if (i > 0)
						sb.append(",");
					FuncionarioDelegacia f = pendentes.get(i);
					sb.append("{").append("\"idFuncionario\":").append(f.getIdFuncionario()).append(",")
							.append("\"nome\":\"").append(f.getNome()).append("\",").append("\"login\":\"")
							.append(f.getLogin()).append("\",").append("\"cargo\":\"").append(f.getCargo()).append("\"")
							.append("}");
			...
			case "aprovar" -> {
				...
				funcionarioDAO.atualizarStatus(idFuncionario, StatusCadastro.APROVADO);
				enviarResposta(ex, 200, "{\"sucesso\":true}");
			}
			case "recusar" -> {
				int idFuncionario = Integer.parseInt(extrairCampo(corpo, "idFuncionario"));
				funcionarioDAO.atualizarStatus(idFuncionario, StatusCadastro.RECUSADO);
				enviarResposta(ex, 200, "{\"sucesso\":true}");
			}
			case "resetarSenha" -> {
				int idFuncionario = Integer.parseInt(extrairCampo(corpo, "idFuncionario"));
				String novaSenha = extrairCampo(corpo, "novaSenha");
				funcionarioDAO.resetarSenha(idFuncionario, novaSenha);
				enviarResposta(ex, 200, "{\"sucesso\":true}");
			}
